...
import type { ReviewFinding, ReviewLocation } from "../agent-runtime/types.js";
		...
		Type.Object(
			{
				path: Type.String({
					minLength: 1,
					description:
						"Repository-relative path of the file containing the location.",
				}),
				startLine: Type.Integer({
					minimum: 1,
					description: "1-based first line of the source location, inclusive.",
				}),
				endLine: Type.Optional(
					Type.Integer({
						minimum: 1,
						description: "1-based last line of the source location, inclusive.",
					}),
				),
			},
			{
				description: "Exact source range supporting the review finding.",
			},
		),
		{
			minItems: 1,
			description:
				"One or more exact changed-file locations supporting the same finding.",
		},
	),
	problem: Type.String({
		minLength: 1,
...
type ReviewFindingParams = Static<typeof reviewFindingSchema>;
...
export const createReviewFindingTool = (
	...
	return defineTool({
		...
		description:
			"Submit one evidence-based code review finding with precise source locations and specific fix guidance.",
		...
		promptGuidelines: [
			...
			"Every location must identify an existing file and exact line range in the repository.",
			"Use multiple locations when the same finding is supported by multiple places.",
			"Only submit findings that are directly observable in the code or can be confirmed through a read-only investigation. Do not invent findings.",
			...
			"After submitting one or more findings, respond exactly: Review complete.",
		],
		parameters: reviewFindingSchema,
		async execute(_toolCallId, params: ReviewFindingParams) {
...
const validateFinding = async (
	finding: ReviewFindingParams,
	repoDir: string,
	submittedFindings: Set<string>,
): Promise<void> => {
	for (const location of finding.locations) {
		await validateLocation(location, repoDir);
	}
...
const validateLocation = async (
	...
	if (location.endLine && location.startLine > location.endLine) {
		throw new Error("Location startLine must not exceed endLine.");
	}

	const filePath = resolve(repoDir, location.path);
	...
	if (
		...
		throw new Error(
			`Location path is outside the repository: ${location.path}`,
		);
	}
	...
	try {
		...
		throw new Error(`Location file does not exist: ${location.path}`);
	}
	...
	if (location.startLine > lines.length) {
		throw new Error(
			`Location ${location.path}:${location.startLine} is outside the file.`,
		);
	}

	if (location.endLine && location.endLine > lines.length) {
		throw new Error(
			`Location ${location.path}:${location.endLine} is outside the file.`,
		);
	}
};

const findingKey = (
	finding: Pick<ReviewFinding, "title" | "locations">,
): string =>
	JSON.stringify({
		title: finding.title,
